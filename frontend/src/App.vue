    ...
    <header class="top-bar">
      ...
      <div class="status-row">
        ...
        <span class="prod-count">
          今日产量({{ shiftName }}): {{ store.data ? store.shiftProduction : 0 }}
        </span>
        <span v-if="store.stale" class="stale-mini">读数已过期</span>
      </div>
    </header>
    <LineStatusBar />
    <div class="main-grid">
...
import LineStatusBar from './components/LineStatusBar.vue'
import { useFactoryStore } from './store/factory'
import { SHIFTS } from './types'
const store = useFactoryStore()
const shiftName = computed(() => SHIFTS.find(s => s.id === store.selectedShift)?.name ?? '')
...
.stale-mini{color:#fbbf24;font-size:11px;background:#b4530922;border:1px solid #b4530966;padding:1px 8px;border-radius:999px}
